...
RUN apt-get update && \
    apt-get install -y curl wget file tar bzip2 gzip unzip bsdmainutils python util-linux ca-certificates binutils bc jq tmux netcat lib32gcc1 lib32stdc++6 libsdl2-2.0-0
...
CMD ./ShooterGameServer TheIsland?listen?SessionName=hihouhou_server?ServerPassword=${ARK_SESSION_PASSWORD}?ServerAdminPassword=${ARK_ADMIN_PASSWORD}?Port=${ARK_PORT}?QueryPort=${ARK_QUERY_PORT} -server -log -epiconly
